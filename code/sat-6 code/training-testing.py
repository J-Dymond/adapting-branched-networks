...
for run in range(args.runs):
    ...
    for epoch in range(nb_epochs):
        ...
        if val_accuracy[epoch] > best_accuracy:
            torch.save(model.state_dict(), model_directory + 'best-'+args.model+'-'+args.data+'-'+str(run)+'.pth')
    ...
    torch.save(model.state_dict(), model_directory + 'epoch'+str(epoch)+'-'+args.model+'-'+args.data+'-'+str(run)+'.pth')
...
save_string = metric_directory+'parameter-counts-ResNet'+str(n_layers)+'.npy'
